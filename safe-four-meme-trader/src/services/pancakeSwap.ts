...
export class PancakeSwapService {
  ...
  private static async checkTokenLiquidity(tokenAddress: string): Promise<boolean> {
    try {
      // PRIORITIZE V2 - Try router check first (more reliable than factory check)
      try {
        console.log(`🔍 Checking V2 liquidity via router for token ${tokenAddress.slice(0, 8)}...`);
        const amounts = await publicClient.readContract({
          address: CONTRACT_ADDRESSES.PANCAKESWAP_V2_ROUTER as `0x${string}`,
          abi: PANCAKESWAP_V2_ROUTER_ABI,
          functionName: 'getAmountsOut',
          args: [BigInt(1e18), [CONTRACT_ADDRESSES.WBNB as `0x${string}`, tokenAddress as `0x${string}`]]
        });

        if (amounts && amounts.length > 1 && amounts[1] > 0n) {
          console.log(`✅ Token has V2 liquidity on PancakeSwap (router check passed)`);
          return true;
        }
      } catch (routerError) {
        console.log(`⚠️ V2 router check failed: ${(routerError as Error).message}`);
      }

      // Fallback to factory check
      try {
        ...
        console.log(`⚠️ Factory check failed: ${(factoryError as Error).message}`);
      }

      // If both checks fail, assume V2 might still work (many tokens have liquidity even if checks fail)
      console.log(`⚠️ V2 checks failed, but will still try V2 first in router selection`);
      return true; // Return true to allow V2 attempts
  ...
  private static async findBestRoute(tokenAddress: string, bnbAmount: number): Promise<{
    ...
    try {
      ...
      // PRIORITIZE V2 - Try V2 first with better error handling
      try {
        console.log(`🔍 Checking V2 liquidity for token ${tokenAddress.slice(0, 8)}...`);
        const amounts = await publicClient.readContract({
          address: CONTRACT_ADDRESSES.PANCAKESWAP_V2_ROUTER as `0x${string}`,
          abi: PANCAKESWAP_V2_ROUTER_ABI,
          functionName: 'getAmountsOut',
        ...
        if (amounts && amounts.length > 1 && amounts[1] > 0n) {
          console.log(`✅ V2 route found: ${Number(amounts[1]) / 1e18} tokens`);
          return {
            version: 'V2',
            path,
          ...
          console.log(`⚠️ V2 route returned 0 tokens, trying V3`);
        }
      } catch (v2Error) {
        console.log(`⚠️ V2 route check failed: ${(v2Error as Error).message}`);
        console.log(`🔄 Falling back to V3...`);
      }

      // Only use V3 if V2 truly fails
      console.log(`🔄 Using V3 router as fallback`);
      return {
        version: 'V3',
        path,
  ...
  private static async findBestSellRoute(tokenAddress: string, tokenAmount: number): Promise<{
    ...
    try {
      ...
      // PRIORITIZE V2 - Try V2 first with better error handling
      try {
        console.log(`🔍 Checking V2 sell liquidity for token ${tokenAddress.slice(0, 8)}...`);
        const amounts = await publicClient.readContract({
          address: CONTRACT_ADDRESSES.PANCAKESWAP_V2_ROUTER as `0x${string}`,
          abi: PANCAKESWAP_V2_ROUTER_ABI,
          functionName: 'getAmountsOut',
        ...
        if (amounts && amounts.length > 1 && amounts[1] > 0n) {
          console.log(`✅ V2 sell route found: ${Number(amounts[1]) / 1e18} BNB`);
          return {
            version: 'V2',
            path,
            estimatedOutput: Number(amounts[1]) / 1e18,
          ...
          console.log(`⚠️ V2 sell route returned 0 BNB, trying V3`);
        }
      } catch (v2Error) {
        console.log(`⚠️ V2 sell route check failed: ${(v2Error as Error).message}`);
        console.log(`🔄 Falling back to V3 for selling...`);
      }

      // Only use V3 if V2 truly fails
      console.log(`🔄 Using V3 router as fallback for selling`);
      return {
        version: 'V3',
        path,
