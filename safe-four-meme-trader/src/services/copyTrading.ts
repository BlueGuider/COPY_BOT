...
export interface CopyTrade {
  ...
  userId: string; // User who initiated the copy trade
  buyPriceBNB?: number; // Buy price in BNB for profit calculation
...
export class CopyTradingService {
  ...
  private static async processBlock(blockNumber: number): Promise<void> {
    try {
      ...
      // Handle block not found errors gracefully
      if (error instanceof Error && error.message.includes('Block at number') && error.message.includes('could not be found')) {
        console.log(`⚠️ Block ${blockNumber} not found yet, skipping...`);
        return;
      }
    ...
    try {
      ...
      if (!platformInfo.isTradeable) {
        if (platformInfo.platform === 'DEX') {
          ...
          // Continue with the copy trade even if isTradeable is false for DEX
        } else {
      ...
      if (tradeInfo.type === 'SELL') {
        // SMART SELL STRATEGY: Don't copy sells immediately, check profit first
        console.log(`   📊 SELL transaction detected - implementing smart sell strategy...`);
        console.log(`   📊 Target wallet sold ${tradeInfo.tokenAmount?.toFixed(2) || 'unknown'} tokens`);
        
        // Check if we have this token in our holdings
        const hasTokenHoldings = await this.checkTokenHoldings(tradeInfo.tokenAddress, userId);
        if (!hasTokenHoldings) {
          console.log(`   ❌ No token holdings found for ${tradeInfo.tokenAddress.slice(0, 8)}..., skipping sell`);
          return;
        }
        
        // Check current profit/loss
        const profitInfo = await this.checkTokenProfit(tradeInfo.tokenAddress, userId);
        console.log(`   📈 Current profit: ${profitInfo.profitPercent.toFixed(2)}%`);
  ...
  private static async determineTradingPlatform(tokenAddress: string): Promise<{
    ...
    try {
      ...
      if (tokenInfo.tokenManager === '0x0000000000000000000000000000000000000000') {
        console.log(`      ✅ Token ${tokenAddress.slice(0, 8)}... has zero token manager - MIGRATED TO DEX - WILL COPY TRADE`);
        return {
          platform: 'DEX',
  ...
  private static async executeCopyTrade(userId: string, copyTrade: CopyTrade): Promise<void> {
    try {
      ...
      if (copyTrade.tradeType === 'BUY') {
        ...
        if (platformInfo.platform === 'DEX') {
          // Use PancakeSwap for migrated tokens
          console.log(`🔄 Using PancakeSwap for migrated token buy`);
          result = await PancakeSwapService.buyTokens(
          ...
          // Use four.meme for regular tokens
          console.log(`🔄 Using four.meme for regular token buy`);
        ...
        if (result.success) {
          ...
          if (result.data?.successCount && result.data?.totalWallets) {
          ...
          try {
            ...
            if (priceResult.success && priceResult.data) {
              ...
              // Store buy price in copy trade for profit calculation
              copyTrade.buyPriceBNB = priceResult.data.avgPrice;
              console.log(`   💰 Stored buy price: ${priceResult.data.avgPrice.toFixed(8)} BNB`);
  ...
  // Smart Sell Strategy Helper Methods

  /**
   * Check if we have holdings of a specific token
   */
  private static async checkTokenHoldings(tokenAddress: string, userId: string): Promise<boolean> {
    try {
      // Get wallet addresses for this user
      const wallets = await WalletService.getWallets(userId);
      if (!wallets.success || !wallets.data?.wallets) {
        return false;
      }
      ...
      for (const wallet of wallets.data.wallets) {
        try {
          ...
          if (balance > 0) {
            console.log(`   💰 Found ${balance} tokens in wallet ${wallet.address.slice(0, 8)}...`);
            return true;
          }
        } catch (error) {
  ...
  /**
   * Check current profit/loss for a token
   */
  private static async checkTokenProfit(tokenAddress: string, userId: string): Promise<{
    profitPercent: number;
    buyPrice: number;
    currentPrice: number;
  }> {
    try {
      // Get the buy price from our active trades
      const activeTrades = Array.from(this.activeTrades.values());
      const tokenTrade = activeTrades.find(trade => 
        trade.tokenAddress.toLowerCase() === tokenAddress.toLowerCase() && 
        trade.userId === userId
      );

      console.log(`   🔍 Looking for buy price in ${activeTrades.length} active trades...`);
      if (tokenTrade) {
        console.log(`   📊 Found trade: buyPriceBNB=${tokenTrade.buyPriceBNB}, tradeType=${tokenTrade.tradeType}`);
      }

      if (!tokenTrade || !tokenTrade.buyPriceBNB) {
        // Try to get buy price from price tracking service
        console.log(`   🔍 No buy price in active trades, checking price tracking service...`);
        const trackedTokens = this.priceTrackingService.getTrackedTokens(userId);
        const trackingData = trackedTokens.find(token => 
          token.tokenAddress.toLowerCase() === tokenAddress.toLowerCase()
        );
        
        if (trackingData && trackingData.buyPrice) {
          console.log(`   ✅ Found buy price from tracking: ${trackingData.buyPrice} BNB`);
          const buyPrice = trackingData.buyPrice;
          ...
          if (priceResult.success && priceResult.data) {
            ...
            console.log(`   📊 Buy price: ${buyPrice.toFixed(8)} BNB, Current price: ${currentPrice.toFixed(8)} BNB, Profit: ${profitPercent.toFixed(2)}%`);
            return { profitPercent, buyPrice, currentPrice };
          }
        }
        
        console.log(`   ⚠️ No buy price found for token ${tokenAddress.slice(0, 8)}..., assuming 0% profit`);
        return { profitPercent: 0, buyPrice: 0, currentPrice: 0 };
      }
      ...
      if (!priceResult.success || !priceResult.data) {
        console.log(`   ⚠️ Could not get current price for ${tokenAddress.slice(0, 8)}..., assuming 0% profit`);
        return { profitPercent: 0, buyPrice: tokenTrade.buyPriceBNB, currentPrice: 0 };
      }

      const currentPrice = priceResult.data.avgPrice;
      const buyPrice = tokenTrade.buyPriceBNB;
      const profitPercent = ((currentPrice - buyPrice) / buyPrice) * 100;
      ...
      console.error('Error checking token profit:', error);
      return { profitPercent: 0, buyPrice: 0, currentPrice: 0 };
    }
  }

  /**
   * Schedule a smart sell with 2-minute hold period
   */
  private static async scheduleSmartSell(
    tokenAddress: string, 
    userId: string, 
  ...
  /**
   * Execute the smart sell
   */
  private static async executeSmartSell(
    tokenAddress: string, 
    userId: string, 
    _config: CopyTradingConfig
  ): Promise<void> {
    try {
      console.log(`   🚀 Executing smart sell for ${tokenAddress.slice(0, 8)}...`);
      
      // Determine platform
      const platformInfo = await this.determineTradingPlatform(tokenAddress);
      
      if (platformInfo.platform === 'DEX') {
        // Use PancakeSwap for migrated tokens
        const result = await PancakeSwapService.sellTokens(tokenAddress, 100, userId, 5.0); // Sell 100% of holdings
        if (result.success) {
          console.log(`   ✅ Smart sell executed successfully: ${result.data?.txHash}`);
        } else {
          console.log(`   ❌ Smart sell failed: ${result.error}`);
        }
      } else {
        // Use four.meme for regular tokens
        const result = await TradingService.sellTokens(tokenAddress, 100, userId); // Sell 100% of holdings
        if (result.success) {
          console.log(`   ✅ Smart sell executed successfully: ${result.data?.txHash}`);
        } else {
          console.log(`   ❌ Smart sell failed: ${result.error}`);
      ...
      console.error('Error executing smart sell:', error);
    }
  }
}
